...
<%--<table cellpadding="0" cellspacing="0" width="100%" border="0">
    <tr>
        <td class="title" colspan="3">
            <span class="num">
                <%=SN++ %>.</span> <b>创建.(<%=Advice.Title %>
                    于<%=FormatDate(Advice.Created) %>)</b>
        </td>
    </tr>
</table>--%>
<hr />
...
<table cellpadding="0" cellspacing="0" width="100%" border="0">
    <tr>
        <td class="title" colspan="3">
            <span class="num">
                <%=SN++ %>.</span><b> 转办：<%=GetTypeName(tran.FromTypeID)%>-><%=GetTypeName(tran.ToTypeID) %>.(<%= GetUserName(tran.UserID)%>于<%=FormatDate(Advice.Created)%>)</b>
        </td>
    </tr>
...
<table cellpadding="0" cellspacing="0" width="100%" border="0">
    <tr>
        <td class="title" colspan="3">
            <span class="num">
                <%=SN++ %>.</span><b><%=Advice.StateText %>.<%=Advice.Name %>(admin 于<%=FormatDate(Advice.Created)%>)
                </b>
        </td>
    </tr>
</table>
