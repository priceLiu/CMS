...
<asp:Content ID="We7Content" ContentPlaceHolderID="MyContentPlaceHolder" runat="server">
	...
	<script type="text/javascript">
	    $(document).ready(function () {
	        ...
	        //			    $("#gotomobile").attr("href", "?file=default").text("编辑PC版");
	        if (QueryString("file") == "mobile")
	            $("#gotomobile").hide();
    ...
    <a style="float: right; margin-right: 30px;" id="gotodefault" href="?file=default">PC版</a>
    <%if (HasMobileGroup)
      {%>
    <a style="float: right; margin-right: 30px;" id="gotomobile" href="?file=mobile">手机版</a>
      <%}
       %>
       <% if (HasTabletGroup)
          {%>
  
    <a style="float: right; margin-right: 30px;" id="gototablet" href="?file=tablet">平板版</a>
          <%} %>
	...
	<div class="toolbar" style="display: <%=ToolbarVisible%>">
		...
		<li class="smallButton8"><a href="TemplateFileDetail.aspx" id="createVisualTemplate"
			class="hint" title="新建可视化模板，可视可拖拽">新建可视化模板▼</a> </li>
		<li class="smallButton4">
		...
		<li class="smallButton6"><a href="Javascript:void(0);" onclick="importTemplate();">模板文件目录</a>
		</li>
		<li class="smallButton8">
			<asp:LinkButton runat="server" ID="CreateMapLink" OnClick="CreateMapLink_Click" class="hint"
				title="新建模板后，请重建模板索引，以便列表调用">重建模板索引图</asp:LinkButton></li>
		<%-- <li class="smallButton8">
		...
		<li class="smallButton8">
			<asp:LinkButton runat="server" ID="CreateWidgetIndex" OnClick="CreateWidgetIndex_Click"
				class="hint" title="新建或下载部件后，请重建部件索引，以便列表调用">重建部件索引</asp:LinkButton></li>
		<li class="smallButton8">
			<asp:LinkButton runat="server" ID="CreateThemeIndex" OnClick="CreateThemeIndex_Click"
				class="hint" title="新建主题后，请重建主题索引，以便列表调用">重建主题索引</asp:LinkButton></li>
		<li class="smallButton4">
