...
<div style="min-height: 35px; width: 100%">
    <asp:GridView ID="DataGridView" runat="server" AutoGenerateColumns="false" CssClass="List"
        ...
        <Columns>
            ...
            <asp:TemplateField>
                <HeaderTemplate>
                    <asp:HyperLink ID="OrderByIndexHyperLink" NavigateUrl="javascript:orderByIndex()" runat="server"><span style="text-decoration:underline">索引</span></asp:HyperLink>
                </HeaderTemplate>
                <ItemTemplate>
            ...
            $.ajax({
                ...
                success: function(_result_) {
                    ...
                    if(json.result) {
                        ...
                        alert("排序过程中出现异常，请联系管理员处理！");
                    }
                }
            });
        } else
            alert("已经是当前页面第一位了，不能在上移了！");
    }
