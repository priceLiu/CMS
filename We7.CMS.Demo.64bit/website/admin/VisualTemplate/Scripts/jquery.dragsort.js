...
function EditWidgetCode(id) {
    ...
    if (value) {
        ...
        if (value.indexOf("静态类")!=-1) {
            ...
            , '<%=Request["ctr"] %>'
            , 'scrollbars=no;resizable=no;help=no;status=no;center=1; dialogHeight=550px;dialogwidth=850px;');
        }
        else {
            r = window.showModalDialog(
        ...
        //注释的原因：用户关闭窗口时，r为空，此时不应该进行后面的操作  ？？为了兼容chrome
        //if (!r) {
          //  r = window.ReturnValue; 
        //}
        if (r != null && r != '') {
            ...
            //判断是否是预览部件操作
            if (json.data.atts.filename.toString().indexOf("tempwidget") > 0) {
                parent.WidgetController.previewapp(json.data.atts);
            }else
