...
function articleOptionCheck(userCtrlID) {
    ...
    var descript = document.getElementById(userCtrlID + "_DescriptionTextBox");
    if (descript.value != '') {
        var strlength = 0;
        for (i = 0; i < descript.value.length; i++) {
            if (!/[u00-uFF]/.test(descript.value.charAt(i)) == true)
...
//        if (strlength < 50) {
//            alert("摘要内容至少五十个字符！");
//            document.getElementById(userCtrlID + "_DescriptionTextBox").focus();
//            return false;
//        }
    }
}
