function checkKey(name, el) {
    jQuery.get("/Admin/Ajax/IPStrategyCheck.aspx?key=" + encodeURI(name) + "&type=1&r=" + new Date().getTime(), function (result) {
...
function checkName(name, el) {
    jQuery.get("/Admin/Ajax/IPStrategyCheck.aspx?key=" + encodeURI(name) + "&type=0&r=" + new Date().getTime(), function (result) {
        el.style.display = "none";
        if (typeof result == "string") result = eval("(" + result + ")");
        if (result.responseText == "true") {
            el.style.display = "";
