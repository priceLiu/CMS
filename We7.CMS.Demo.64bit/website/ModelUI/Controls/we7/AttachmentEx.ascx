...
<script type="text/javascript">
    $(function() {
        new AttachmentBuilder({ TriggerElement: $('#<%=bttnUploader.ClientID %>'), Src: "<%=Src %>", FrameWidth: <%=FrameWidth %>, FrameHeight: <%=FrameHeight %>, HiddenValue: $("#<%=hfValue.ClientID %>"), Ct: $("#<%=this.ClientID %>"), ArticleID: '<%=ArticleID %>', FileExtension: '<%=FileExtension %>', FileNamePrefix: getFileNamePrefix ,OnCompleted:On_Completed});
    });
