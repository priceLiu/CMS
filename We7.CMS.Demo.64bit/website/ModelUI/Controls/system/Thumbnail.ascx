...
<div>
    <div style="display: none">
        <asp:TextBox ID="TxtPath" runat="server" Width="200" onblur="this.className='input_blur'" onfous="this.className='input_focus'"></asp:TextBox></div>
    ...
    {
        var url = '/ModelUI/Controls/system/page/ThumbnailServer.ashx?Type=BindImgList&ID=<%=ArticleID %>&h=' + Math.random();
        var myAjax = jQuery.get(url, showResponseshow);   //new Ajax.Request(url, { method: 'get',parameters: pars, onComplete:showResponseshow,asynchronous:false  } ); 
    }
    function DelFile(str)
    {
        var url = '/ModelUI/Controls/system/page/ThumbnailServer.ashx?Type=DelFile&ID=<%=ArticleID %>&fileName=' + str + '&h=' + Math.random();
        var myAjax = jQuery.get(url, showResponseDel);   //new Ajax.Request(url, { method: 'get',parameters: pars, onComplete:showResponseDel,asynchronous:false  } );  
    }
