<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="RelationSelectEx.ascx.cs"
    Inherits="We7.Model.UI.Controls.system.RelationSelectEx" %>
...
<div id="ctAdd" style="margin-left: 100px; padding: 3px; border: solid 1px #e0e0e0;
    ...
    <script type="text/javascript">
        function Add() {
            ...
            var newIDkey = '<%=Control.Params["valuefield"] %>';
            var newNamekey = '<%=Control.Params["textfield"] %>';
            $.ajax({
                type: 'POST',
                url: '/ModelUI/Controls/page/RelationSelectEx.aspx',
                data:'NewIDkey='+newIDkey+'&NewNamekey='+newNamekey+ '&NewID=' + newID + '&NewName=' + newName + '&Model=<%=Control.Params["table"] %>',
                cache: false,
